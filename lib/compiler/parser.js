...
parser.prototype.parseSubExpression = function(tokens, current) {
  return [current + 1, {
    type: 'SubExpression',
    value: tokens[current].value
  }];
...
parser.prototype.parseExpression = function(tokens, current) {
  ...
  var parseExpToken = function(token) {
    switch (token.type) {
      ...
      case 'sub-expression': [, token] = self.parseSubExpression([token], 0); break;
...
parser.prototype.parseTag = function(tokens, current) {
  ...
  if (!token) {
    var err = new Error('invalid open tag');
...
parser.prototype.parseToken = function(tokens, current) {
  ...
  if (token.type === 'sub-expression') {
    return this.parseSubExpression(tokens, current);
  }
  var err = new TypeError(token.type);
