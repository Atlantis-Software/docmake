...
container.prototype.break = function(height) {
  ...
  height = height - this.borders.top - this.margins.top;
  var currentHeight = 0;
  ...
  if (lineBreak > 50) {
    fitLine = new line();
    var indexItem = 0;
    while ((indexItem < currentLine.content.length) && breakable) {
      ...
      if (lineItem.spacer) {
        ...
        if (item.height) {
          item.height = 0;
          heightSpacers.push({current: lineItem, next: item});
        }
      } else if (lineItem.height < lineBreak) {
        item = lineItem;
    ...
    if (breakable) {
      ...
      // update height
      heightSpacers.forEach(function(spacers) {
        spacers.next.height = spacers.current.height - fitLine.height;
        spacers.current.height = fitLine.height;
      });
      fitLine.update();
      currentLine.update();
    } else {
