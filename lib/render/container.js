...
var container = function(node, size) {
  ...
  this.height = size[1];
  this.margins = {
    left: node.margins.left,
    top: node.margins.top,
    bottom: node.margins.bottom,
    right: node.margins.right
  };
  this.borders = {
    left: node.borders.left,
    top: node.borders.top,
    bottom: node.borders.bottom,
    right: node.borders.right
  };
...
container.prototype.break = function(height) {
  // remove the top border of the wanted height
  height -= this.borders.top;
  var currentHeight = 0;
  ...
  if (lineBreak > 50) {
    fitLine = new line();
    var indexItem = 0;
    while ((fitLine.height < lineBreak) && (indexItem < currentLine.content.length) && breakable) {
      ...
      if (lineItem instanceof container) {
        ...
        if (lineItem.height < (lineBreak - fitLine.height)) {
          item = lineItem;
          removeable.push(indexItem);
        } else {
          item = false;
        }
      }
    ...
    if (breakable) {
      removeable.forEach(function(indexItem) {
  ...
  if (_.isNull(lineCutIndex) && _.isNull(fitLine)) {
    return false;
  }
