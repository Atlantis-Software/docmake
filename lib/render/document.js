...
document.prototype.closeDoc = function() {
  this.currentPage.end();
  this.header = null;
  this.footer = null;
  var options = {
...
document.prototype._setStyle = function(node) {
  ...
  node.transforms.forEach(function(transform) {
    if (transform.scale) {
  ...
  if (node.fillColor) {
    if (node.fillColor === "none") {
      node.fillColor = false;
    } else {
      this.doc.fillColor(node.fillColor);
    }
  }
  if (node['stroke-width']) {
    this.doc.lineWidth(node['stroke-width']);
  }
};
...
document.prototype._draw = function(node) {
  ...
  if (node.stroke) {
    this.doc.strokeColor(node.stroke);
  }
  if (node.stroke && node.fillColor) {
...
document.prototype._renderPage = function(page) {
  ...
  page.container.forEach(function(node) {
    if (node.tag === "text") {
      ...
      node.childs.forEach(function(elm) {
        elm.transforms.unshift({translate: {x: node.x, y: node.y}});
      });
      self._renderPage({container: node.childs});
    } else {
