...
module.exports = function(document, parent, node) {
  var svg = null;
  try {
    svg = new xmldoc.XmlDocument(node.svg);
  } catch (err) {
  ...
  var SVGstyle = function(element, defaultStyle) {
    ...
    function parseTransformation(transformString) {
      ...
      if (transformString) {
        ...
        for (var i in transformString = transformString.match(/(\w+\((\-?\d+\.?\d*e?\-?\d*,?)+\))+/g)) {
          ...
          switch (transformFunctionName) {
            case 'rotate':
              transformObj.rotate = {
                ...
                x: transformFunction[1] ? transformFunction[1] + offsetX : offsetX,
                y: transformFunction[2] ? transformFunction[2] + offsetY : offsetY
              };
              break;
          }
  ...
  var parse = function(element, defaultStyle) {
    ...
    switch (element.name) {
      case 'g':
        var gStyle = Object.create(defaultStyle);
        if (element.attr.fill) {
          gStyle.fill = getColor(element.attr.fill);
        }
        if (element.attr.stroke) {
          gStyle.stroke = getColor(element.attr.stroke);
        }
        if (element.attr['stroke-width']) {
          gStyle['stroke-width'] = parseFloat(element.attr['stroke-width']);
        }
        ...
        break;
      case 'rect':
        elm.rect = SVGstyle(element, defaultStyle);
        elm.rect.width = element.attr.width ? parseFloat(element.attr.width) : 0;
        elm.rect.height = element.attr.height ? parseFloat(element.attr.height) : 0;
        ...
        break;
      case 'circle':
        elm.circle = SVGstyle(element, defaultStyle);
        elm.circle.radius = element.attr.r ? parseFloat(element.attr.r) : 0;
        ...
        break;
      case 'ellipse':
        elm.ellipse = SVGstyle(element, defaultStyle);
        elm.ellipse.radius = {
      ...
      case 'line':
        ...
        elm.line = SVGstyle(element, defaultStyle);
        elm.line.to = {
          x: x2 - x1,
          y: y2 - y1
        };
  ...
  var root = {
    fill: svg.attr.fill ? getColor(svg.attr.fill) : 'none',
    stroke: svg.attr.stroke ? getColor(svg.attr.stroke) : getColor('black'),
