...
module.exports = function(Processor, document, parent, node) {
  ...
  // fusion rows' borders
  node.table.body.forEach(function(rowNode, rowIndex) {
    // fusion top and bottom borders on rows
    var rowTopBorderFusion = node.borders.top;
    if (rowIndex !== 0) {
      rowTopBorderFusion = node.table.body[rowIndex - 1].borders.bottom;
    }
    if (rowTopBorderFusion >= rowNode.borders.top) {
      rowNode.borders.top = 0;
    } else {
      rowNode.borders.top = rowNode.borders.top - rowTopBorderFusion;
    }

    // fusion left borders on rows
    if (node.borders.left >= rowNode.borders.left) {
      rowNode.borders.left = 0;
    } else {
      rowNode.borders.left = rowNode.borders.left - node.borders.left;
    }

    //fusion rows right border with table right border
    if (node.borders.right >= rowNode.borders.right) {
      rowNode.borders.right = 0;
    } else {
      rowNode.borders.right = rowNode.borders.right - node.borders.right;
    }

    //fusion last row bottom border with table bottom border
    if (rowIndex === (node.table.body.length - 1)) {
      if (node.borders.bottom >= rowNode.borders.bottom) {
        rowNode.borders.bottom = 0;
      } else {
        rowNode.borders.bottom = rowNode.borders.bottom - node.borders.bottom;
      }
    }
  });
  ...
  node.table.body.forEach(function(rowNode, rowIndex) {
    ...
    // for each column
    var colIndex = 0;
    var colBorderFusion = node.borders.left + rowNode.borders.left;
    if (rowNode.height) {
      rowNode.availableRowHeight = rowNode.height - rowNode.margins.top - rowNode.margins.bottom - rowNode.borders.top - rowNode.borders.bottom;
    }
    rowNode.row.forEach(function(cellNode) {
      cellNode.id = 'cell' + colIndex + '_' + rowIndex;
      if (rowNode.availableRowHeight && rowNode.availableRowHeight > cellNode.height) {
        cellNode.height = rowNode.availableRowHeight;
      }
      ...
      // fusion top row border and top cells borders
      var cellTopBorderFusion = rowNode.borders.top;
      if (rowIndex === 0) {
        cellTopBorderFusion += node.borders.top;
      } else {
        cellTopBorderFusion += node.table.body[rowIndex - 1].borders.bottom;
      }
      if (cellTopBorderFusion >= cellNode.borders.top) {
        cellNode.borders.top = 0;
      } else {
        cellNode.borders.top = cellNode.borders.top - cellTopBorderFusion;
      }

      // fusion bottom cells borders
      var cellBottomBorderFusion = rowNode.borders.bottom;
      if (rowIndex === (node.table.body.length -1)) {
        cellBottomBorderFusion += node.borders.bottom;
      } else {
        cellBottomBorderFusion += node.table.body[rowIndex + 1].borders.top;
      }
      if (cellBottomBorderFusion >= cellNode.borders.bottom) {
        cellNode.borders.bottom = 0;
      } else {
        cellNode.borders.bottom = cellNode.borders.bottom - cellBottomBorderFusion;
      }

      // fusion left borders on cells
      if (colBorderFusion >= cellNode.borders.left) {
        cellNode.borders.left = 0;
      } else {
        cellNode.borders.left = cellNode.borders.left - colBorderFusion;
      }
      colBorderFusion = cellNode.borders.right;
      // fusion last cell right border with row right border and table right border
      if (colIndex === colCount - 1 || colIndex + cellNode.colspan === colCount) {
        var cellRightBorderFusion = rowNode.borders.right + node.borders.right;
        if (cellRightBorderFusion >= cellNode.borders.right) {
          cellNode.borders.right = 0;
        } else {
          cellNode.borders.right = cellNode.borders.right - cellRightBorderFusion;
        }
      }
  ...
  node.widths.forEach(function(width, colIndex) {
    ...
    if (width === 'auto' && matrixContainer[colIndex]) {
      var colWidths = [];
  ...
  node.widths.forEach(function(width, colIndex) {
    ...
    if (width === '*' && matrixContainer[colIndex]) {
      // set stars width
  ...
  for (var rowIndex = 0; rowIndex < rowCount; rowIndex++) {
    ...
    for (let colIndex = 0; colIndex < colCount; colIndex++) {
      let cellContainer = matrixContainer[colIndex][rowIndex];
      let cellNode = matrixNode[colIndex][rowIndex];
      if (cellContainer && cellNode) {
        heights.push(_.max([cellNode.height, cellContainer.getMinHeight()]));
      }
    }
    ...
    var rowContainer = new Container(rowNode, [rowNode.width, rowNode.height]);
    rowContainer.id = 'rowContainer' + rowIndex;
    // set cell height and insert in row container
    for (let colIndex = 0; colIndex < colCount; colIndex++) {
      let cellContainer = matrixContainer[colIndex][rowIndex];
      if (cellContainer) {
        cellContainer.setHeight(rowNode.availableHeight);
  ...
  if (node.repeatHeader) {
    ...
    node.repeatOnBreak = new Container({}, [node.availableWidth, Infinity]);
    var headerHeight = 0;
