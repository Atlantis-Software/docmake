...
var computeInlineStyle = function(node) {
  ...
  inline.padding = validateSpace(node.padding) ? node.padding : null;
  inline.border = validateSpace(node.border) ? node.border : null;
  return inline;
};

var computeClassesStyle = function(classes) {
  var classesStyle = {
    font: null,
    fontSize: null,
    lineHeight: null,
  ...
  classes.forEach(function(className) {
    _.keys(defaultStyles.class[className]).forEach(function(attrName) {
      classesStyle[attrName] = _.clone(defaultStyles.class[className][attrName]);
    });
  });
  return classesStyle;
};
...
var computeInheritanceStyle = function(parent) {
  return {
    ...
    //alignment: defaultStyles.alignment,
...
module.exports = function(parent, node) {
  ...
  var classes = computeClassesStyle(node.class || []);
  ...
  node.paddings = expandSpace(node.padding);
